...
const CoinInfo = ({coin}: CoinInfoProps) => {
  ...
  return (<>{loading ? 
  <div className="w-full h-[85vh] flex justify-center items-center bg-gray-950"><svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="text-white h-full w-full flex justify-center items-center flex-col">
       <div className="w-[80%] h-full pt-20">
        ...
        options={{
         ...
         <div>
          { chartDays.map((day) => {
            return <button className="mt-9 pb-9"><SelectButton  
            key={day.value} 
            onClick={() => setdays(day.value)} 
            selected={day.value === days}>{day.label} </SelectButton></button>
          })}
         </div>
      </div>}</>
    )
